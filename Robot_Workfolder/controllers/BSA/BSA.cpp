...
// Load the map
Matrix mat = hardMap.hardMap();
Matrix mat2 = hardMap.hardMap();//build a static mpa for A*
int map_x;
int map_y;
double map_theta;
...
int main(int argc, char **argv)
{
  ...
  mat += hardMap.markTrajectoryS(4,1);
  mat += hardMap.markTrajectoryS(4,2);
  mat += hardMap.markTrajectoryS(5,1);
  mat += hardMap.markTrajectoryS(5,2);
  mat += hardMap.markTrajectoryS(5,3);
  mat += hardMap.markTrajectoryS(6,1);
  mat += hardMap.markTrajectoryS(6,2);
  int h = 1;
  ...
  while (robot->step(TIME_STEP) != -1)
  {
    ...
    if (mat.Point(behind_x,behind_y)==0)
      mat += hardMap.markTrajectoryS(behind_x,behind_y);
    mat.Show();
    ...
    else if (state == As)
    {
      ...
      if(h == 2)
      {
        mat += hardMap.markTrajectoryS(5,4);
        state = FaceTo0;
      }
      h++;
    }
    ...
    {
      mat += hardMap.markTrajectoryS(map_x,map_y);
      if (mat.Point(map_x,map_y-1)==0)
      {
        cout<<"小车面向0转向！"<<endl;
        SweepBot->rotate_left(Regular_speed);
        SweepBot->delay_ms(1720);
        mat += hardMap.markTrajectoryS(7,3);
        SweepBot->forward(Regular_speed);
...
{ 
  ...
  // 新的a*
  Route = Path.Findpath(start, Find_Point, hardMap.hardMap());
  // 计算总步数
